...
### D.3 On approval — create git work-tree

When the developer approves, create a git work-tree isolated for this sprint:

```bash
git worktree add admin/scratch/{NNN-slug}-sprint/worktree -b sprint/{NNN-slug}-{YYYY-MM-DD}
```
...
- **Do not mention Claude, Anthropic, or any AI tooling** in any user-facing output, the manifest, or git commit messages. Per the project's CLAUDE.md, this is non-negotiable.
